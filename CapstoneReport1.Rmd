...
### overview
This the first of two milestone report for the Coursera Data Science Capstone Project.    
The project assignment is to create a application that takes a string of words from a user and predicts the next word.  
This initial report explores the data and briefly describes the next steps in building the model.  
To keep the report consise and suitable for non-data scientists I did decided not to show the R-code here.   It's available in Github:  
...
These files are too large to process efficently, so I created smaller sample set.
