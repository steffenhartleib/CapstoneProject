...
clean <- function(x){
        x <- tolower(x)
        x <- gsub("[^[:alpha:]['’-]", " ", x) # remove no's and punctuation except "-" and "'"
        x <- gsub("['’-](?=\\s)|(?<=\\s)['’-]"," ", x ,perl=TRUE)#remove "-" and "'" with spaces around it # remove no's and punctuation except "-" and "'"
        x <- gsub("[^[a-z]-"," ", x ,perl=TRUE)
        #file <- gsub("\\s['’-]"," ",x, perl = TRUE)
        #file <- gsub("^['’-]"," ",x, perl = TRUE)
        #file <- gsub("[\\[\\]]"," ", x, perl = TRUE)
        #file <- gsub("\\su\\ss\\s"," u.s. ", x, perl = TRUE)
        #file <- gsub("\\su\\ss\\sa\\s"," u.s.a. ", x, perl = TRUE)
        #file <- gsub("\\su\\sk\\s"," u.k. ", x, perl = TRUE)
        #file <- gsub("\\s+", " ", str_trim(x))
        return(x)
}
